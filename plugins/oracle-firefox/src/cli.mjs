...
try {
  ...
  if (command === "doctor") result = await callBroker("workflow.doctor", {}, { harness: "cli" });
  ...
  else if (command === "reconcile") result = await callBroker("jobs.reconcile", { jobId: args[0] }, { timeoutMs: 120000, harness: "cli" });
    ...
    throw new Error("Usage: oracle-firefox doctor|broker-status|profiles|setup|import-session|projects|find-chats|consult|consult-start|continue-chat|continue-chat-start|status <job-id>|result <job-id>|jobs|watch <job-id> [--jsonl|--notify]|reconcile <job-id>|acknowledge <job-id>|cancel <job-id>|emergency-lock|emergency-unlock");
  }
