...
export const MODEL_STRATEGY_VERSION = "firefox-pro-v2";
  ...
  return page.evaluate((selectors) => {
    ...
    for (const selector of selectors) {
      for (const node of document.querySelectorAll(selector)) {
        if (!visible(node) || node.closest('aside, nav, [data-message-author-role], [data-turn], [data-testid*="history"], [data-testid*="sidebar"]')) continue;
...
export async function ensureModelRequirement(page, requirement = "pro") {
  ...
  const button = await page.evaluateHandle((selectors) => {
    const visible = (node) => {
      if (!(node instanceof HTMLElement)) return false;
      const rect = node.getBoundingClientRect();
      const style = window.getComputedStyle(node);
    ...
    for (const selector of selectors) {
      const candidate = Array.from(document.querySelectorAll(selector)).find((node) =>
        visible(node) && !node.closest('aside, nav, [data-message-author-role], [data-turn], [data-testid*="history"], [data-testid*="sidebar"]')
      );
