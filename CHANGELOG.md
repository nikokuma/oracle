...
- Oracle Firefox 1.6.6: canonicalize ChatGPT's deterministic four-space expansion of contenteditable tab characters across insertion verification, final pre-click fencing, submitted-turn correlation, and durable message hashing while continuing to reject every other semantic difference.
- Oracle Firefox 1.6.5: insert each authorized contenteditable prompt atomically so ChatGPT's ProseMirror cannot consume Markdown-style prefixes through per-keystroke input rules before whole-message verification; wait for the exact retiring broker process to exit before launching an idle-upgrade successor.
- Oracle Firefox 1.6.4: preserve exact authorized paragraph and list boundaries when ChatGPT's ProseMirror composer mixes nested block containers, preventing false pre-submit `COMPOSER_MISMATCH` failures without weakening whole-message verification; advance the durable broker release so idle 1.6.3 workers hand off before the fix is used.
- Oracle Firefox 1.6.3: launch Codex MCP through a plugin-relative executable that resolves Homebrew Node even when the app supplies a restricted `PATH`, and let schema-6 successors claim the broker generation before touching fenced migration rows.
- Oracle Firefox 1.6.3: reject echoed local-evidence protocol examples through terminal-block validation, concrete-field checks, and per-chain nonces; add capability-owned and exact-URL fingerprinted abandonment so users can discard an unwanted input request and release only its FIFO lane without sending or authorizing a replacement.
